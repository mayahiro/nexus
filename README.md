...
Use `--review-dir review/orders` to write an AI review packet containing `compare.json`, `compare.md`, pair and finding decision templates, viewport screenshots, and `review-summary.json`.
With `--manifest`, `--review-dir review/migration` writes manifest-level summaries, `review-index.md`, `review-index.html`, and one review packet directory per page. The HTML index shows screenshots, the first critical and warning findings, and copyable finding decision JSONL stubs for each page.
