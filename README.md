...
Use `--output-decisions-template pair-decisions.todo.jsonl` to write editable JSONL stubs for ambiguous candidates and unmatched old/new nodes. Ambiguous stubs include the old node locator and a unique CSS selector when available, and candidate notes include new refs, locators, selectors, scores, and shared/differing keys. Unmatched old stubs start as unknown `pair` decisions that can become `accepted_removed`; unmatched new stubs start as unknown `accepted_added` decisions. Selector hints are emitted only when Nexus can derive a selector that is unique within the observed compare nodes.
...
Use `--review-dir review/orders` to write an AI review packet containing `REVIEW.md`, `compare.json`, `compare.md`, pair/finding/cluster decision templates, full-page screenshots, best-effort cropped finding screenshots, repeated finding clusters, and `review-summary.json`. Start with `REVIEW.md`; it summarizes the packet, points to the important files, and lists the next commands for validating locator-friendly decisions, materializing them to concrete refs, and normalizing the final JSONL. The review summary also includes pair decision template counts, duplicate-suppression counts, a decision audit summary, and unresolved audit examples when `--decisions-file` is supplied.
With `--manifest`, `--review-dir review/migration` writes a root `REVIEW.md`, manifest-level summaries, `cluster-decisions.todo.jsonl`, `review-index.md`, `review-index.html`, and one review packet directory per page. Start with the root `REVIEW.md` to choose failed pages, critical pages, high pair-decision workload pages, pages with pending/stale/conflicting decisions, and repeated finding clusters; each page packet also includes its own `REVIEW.md`. The root summary and markdown index aggregate page-level pair decision template counts and decision audit counts. The HTML index shows screenshots, repeated finding clusters, cropped finding screenshots, the first critical and warning findings, and copyable finding decision JSONL stubs for each page.
Use `nxctl compare validate-decisions --decisions-file pair-decisions.jsonl --compare-json compare-debug.json --strict` to reject schema-shaped mistakes such as unknown fields, fields that do not apply to the current `kind`, and unsupported `schema_version` values before rerunning compare. Without `--strict`, unknown or unused fields are warnings so AI-authored draft files can still be inspected incrementally.
Use `nxctl compare validate-decisions --decisions-file pair-decisions.selectors.jsonl --compare-json compare-debug.json --old-session old --new-session new --json` to preflight AI-authored `old_selector` and `new_selector` fields before materialization. Selector preflight checks the live DOM selector, requires a single live match, and verifies that the live node maps uniquely back to one compare JSON node; successful selector fields are counted as `selector_preflighted`.
Use `nxctl compare materialize-decisions --decisions-file pair-decisions.locators.jsonl --compare-json compare-debug.json --output pair-decisions.jsonl --json` when AI-authored decisions use `old_locator` or `new_locator` instead of concrete refs. Locator terms are resolved against compare JSON nodes and must match exactly one node per side before the command writes `old` or `new` refs. If the decisions use `old_selector` or `new_selector`, add `--old-session` or `--new-session`; each CSS selector must resolve to one live DOM node that maps uniquely back to a compare JSON node. The JSON report includes `materialized[]` entries explaining the line, side, source field, input value, resolved ref, match strategy, current compare node summary, and selector-backed live node summary.
Use `nxctl compare repair-decisions --decisions-file pair-decisions.jsonl --compare-json compare-debug.json --output pair-decisions.repaired.jsonl --json` when a previously reviewed file contains stale `@e...` refs. Repair only changes existing stale `old` or `new` refs when selector, locator, or fingerprint metadata resolves uniquely to a current compare JSON node; unresolved refs stay unchanged and are reported as warnings. The JSON report includes `repaired[]` entries with the old/new refs, source field, match strategy, current compare node summary, and selector-backed live node summary.
Use `nxctl compare validate-decisions --decisions-file cluster-decisions.todo.jsonl --review-summary review-summary.json` to validate manifest-level repeated finding cluster decisions before normalization.
Use `nxctl compare normalize-decisions --decisions-file pair-decisions.jsonl --compare-json compare-debug.json --review-summary review-summary.json --output pair-decisions.normalized.jsonl` to canonicalize decision tokens, materialize finding-cluster decisions, remove duplicate decisions, and catch stale refs or `finding_id` values.
Use `nxctl compare audit-decisions --decisions-file pair-decisions.jsonl --compare-json compare-debug.json --json` to check whether reviewed decisions were applied, are still pending, became stale, or conflict with another decision. The JSON report includes `entries[]` with `status`, `reason`, `expected`, `actual`, conflict line metadata, and repair hints for stale refs or finding IDs.
Use `--decisions-file pair-decisions.jsonl` to feed reviewed pairing decisions back into compare. Nexus applies high-confidence `pair` entries as deterministic matches and high-confidence `subtree_pair` entries as ordered child, ordered descendant, or opaque subtree matches before automatic matching; `subtree_pair` roots can start as selector, locator, or fingerprint metadata when they are materialized to concrete refs before compare. Tentative or unknown entries remain review notes for future review flows. Findings include stable `finding_id` values, so `accepted_finding` can mark an existing finding as approved info and `regression_finding` can keep an existing finding critical with `decision_kind` metadata. `accepted_removed` and `accepted_added` decisions downgrade matching missing/new findings to info and stamp `decision_kind` on the finding. Validate decisions with `nxctl compare validate-decisions --decisions-file pair-decisions.jsonl --compare-json compare-debug.json`; add `--old-session` or `--new-session` when validating selector-backed decisions. Each JSONL line also matches `docs/ai/compare-decisions.schema.json`.
Use `--node-scope current|actionable|semantic|all` to control observed compare candidates. `current` is the default and preserves existing candidates, `actionable` focuses on controls, and `semantic` includes named or content-bearing semantic nodes such as headings, landmarks, status, and testid-tagged elements.
Use `--node-scope all` for focused migration work where layout wrappers, anonymous containers, or non-semantic DOM structure are part of the expected comparison. It requires `--scope-selector` or both side-specific scope selectors, observes every visible element inside that scope, and adds `structure_key` and `subtree_signature` metadata to nodes, matching debug output, and missing/new findings. Histogram matching can use that metadata as low-occurrence structural anchors without changing the base fingerprint.
