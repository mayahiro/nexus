...
Use `--review-dir review/orders` to write an AI review packet containing `compare.json`, `compare.md`, pair and finding decision templates, full-page screenshots, best-effort cropped finding screenshots, and `review-summary.json`.
With `--manifest`, `--review-dir review/migration` writes manifest-level summaries, `review-index.md`, `review-index.html`, and one review packet directory per page. The HTML index shows screenshots, cropped finding screenshots, the first critical and warning findings, and copyable finding decision JSONL stubs for each page.
