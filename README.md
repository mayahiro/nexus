...
Use `nxctl compare materialize-decisions --decisions-file pair-decisions.locators.jsonl --compare-json compare-debug.json --output pair-decisions.jsonl` when AI-authored decisions use `old_locator` or `new_locator` instead of concrete refs. Locator terms are resolved against compare JSON nodes and must match exactly one node per side before the command writes `old` or `new` refs. If the decisions use `old_selector` or `new_selector`, add `--old-session` or `--new-session`; each CSS selector must resolve to one live DOM node that maps uniquely back to a compare JSON node.
