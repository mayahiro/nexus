...
Use `--output-decisions-template <jsonl>` with `--matching-debug` to write editable `unknown` pair stubs from `matching_debug.ambiguous_candidates`. Pair stubs include `old_locator` when Nexus can infer one, and candidate notes include each new candidate's ref, locator, score, shared keys, and differing keys so an AI can either choose a concrete `new` ref or write a `new_locator` and materialize it.
