...
When a decision uses `old_locator` or `new_locator`, first run `nxctl compare materialize-decisions --decisions-file pair-decisions.locators.jsonl --compare-json compare-debug.json --output pair-decisions.jsonl --json`. Each locator must match exactly one compare JSON node, then the command writes the concrete `old` or `new` ref. When a decision uses `old_selector` or `new_selector`, include `--old-session` or `--new-session`; each selector must match one live DOM node that maps uniquely back to the compare JSON node. Inspect `materialized[]` in the JSON report to see each resolved line's source field, input value, ref, and match strategy.
