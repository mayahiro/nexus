...
func TestNagiApplicationRepresentativeInvocations(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "observe", args: []string{"observe", "--session", "work", "--screenshot", "--recover-target", "--timeout", "45000"}},
		{name: "rightclick", args: []string{"rightclick", "@e1"}},
		{name: "scroll", args: []string{"scroll", "down", "--amount", "400"}},
		{name: "screenshot", args: []string{"screenshot", "capture.png", "--recover-target"}},
...
func TestNagiApplicationUsageVariantContracts(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "observe recovery", path: []string{"nxctl", "observe"}, variantID: "default", syntax: "--session <ID> [--json] [--text] [--tree] [--screenshot] [--full] [--recover-target] [--timeout <MS>]", args: []string{"observe", "--session", "work", "--screenshot", "--recover-target", "--timeout", "45000"}},
